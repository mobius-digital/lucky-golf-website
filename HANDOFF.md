...
## 35. The shot list was wrong by more than half (2026-08-13)

Cole: *"Just tell me every bit of photography you want and I'll grab it."*
Generating that surfaced a real error.

### 35a. 21 → 51

`SHOT-LIST.md` claimed **21 stills**. The true number on built pages is **51**.

The old file was generated from `_src/data/copy/`, walking the copy files
looking for photo briefs. That approach silently missed everything whose brief
lives in a FAMILY or SHARED layer rather than a product file — which is exactly
where the apparel briefs live (§22c: one design section per family, not per
design). Missed entirely:

```
23  apparel heroes   13 polos + 10 hats, one 4:5 on-body each
 8  homepage slots   the why-collage, the finish, the apparel/gear tiles
 1  the tees flat-lay
```

It also listed **two shots for discontinued products** (`lgd01`,
`lgp02-patriot`) whose pages do not exist.

**The fix is the method.** It is now generated by scanning the rendered `.ph`
placeholders in the 58 BUILT pages — a photo brief only counts if it is a hole
on a page a visitor can reach. That cannot drift from the site again, because
it *is* the site.

### 35b. Grouped by shoot, not by page

A photographer does not work page by page. Four sessions:

| | | |
|---|---|---|
| **A** Course day, the clubs | 14 | 13 shootable — the 02 cutout needs a club that does not exist |
| **B** Apparel day, on body | 23 | **unblocks 23 pages at once**, the biggest single win on the list |
| **C** Brand + lifestyle | 13 | Our Story ×4, Ambassador ×1, homepage ×8 |
| **D** Studio flat-lay | 1 | the tees — the only product with NO Shopify photo at all |

Two things were added that the raw briefs did not carry: a **"how to shoot the
gold"** preamble (raking light is load-bearing, flat midday makes it read as
paint — it governs most of session A), and an explicit **BLOCKED** marker on the
02 cutout so nobody books a studio day for a club that does not exist.

### 35c. An internal reference was leaking into a client-facing file

Every one of the 23 apparel briefs ended with *"There is no on-body apparel
photography at all yet (HANDOFF §7b F)."* — true, but a repo cross-reference
repeated 23 times in a document meant to be forwarded to a photographer.
Stripped from the briefs and stated once, at the session, where it is an
argument for doing that session first rather than noise.

**General rule this exposes:** a file that gets sent OUTSIDE the repo cannot
carry `§` references. `SHOT-LIST.md` and `DEVELOPER-HANDOFF.md` are the two
that leave; `DEVELOPER-HANDOFF.md` cites HANDOFF deliberately because its reader
gets the repo too.

### 35d. Verified

`grep -c "^- \[ \]"` = 51, zero `HANDOFF` references remain in the file, and
