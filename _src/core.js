...
/* REV B SWITCH — preview only (2026-08-19). ?rev=b applies the thinned layout
   ...
   appears when ?rev= is present, so a normal visitor never sees it.

   It toggles in place: no navigation, no scroll reset. Because rev B REMOVES
   sections, holding window.scrollY would land you somewhere else on the page —
   so it anchors instead. Before the swap it finds the section nearest the top
   of the viewport and remembers how far into it you are; after the swap it puts
   that same section back at the same offset. If the section you were looking at
   is one rev B hides, it falls back to the next visible one.

   Remove this IIFE and the core.css block together once Cole has picked. */
(function () {
  var rev = new URLSearchParams(location.search).get('rev');
  if (!rev) return;
  var root = document.documentElement;
  function apply(r) { r === 'b' ? root.setAttribute('data-rev', 'b') : root.removeAttribute('data-rev'); }
  apply(rev);

  function anchors() {
    return [].slice.call(document.querySelectorAll(
      'body > section, body > div.mq, body > div.wrap, body > footer, body > nav'))
      .filter(function (el) { return el.offsetParent !== null && el.getBoundingClientRect().height > 20; });
  }
  function swap(next) {
    if (next === rev) return;
    /* what am I looking at, and how far into it? */
    var list = anchors(), mark = null, into = 0;
    for (var i = 0; i < list.length; i++) {
      var t = list[i].getBoundingClientRect().top;
      if (t <= 1) { mark = list[i]; into = -t; } else break;
    }
    rev = next;
    apply(rev);
    history.replaceState(null, '', location.pathname + '?rev=' + rev + location.hash);
    paint();
    if (mark) {
      /* the section may have just been hidden — walk forward to a visible one */
      var el = mark;
      while (el && el.offsetParent === null) el = el.nextElementSibling;
      if (el) {
        var y = el.getBoundingClientRect().top + window.scrollY + (el === mark ? into : 0);
  ...
  function paint() {
    ...
    [].forEach.call(bar.children, function (b) {
      b.toggleAttribute('aria-current', b.dataset.rev === rev);
    });
  }
  document.addEventListener('DOMContentLoaded', function () {
    ...
    [['a', 'A · now'], ['b', 'B · thinned']].forEach(function (o) {
      var b = document.createElement('button');
      b.type = 'button'; b.dataset.rev = o[0]; b.textContent = o[1];
      b.addEventListener('click', function () { swap(o[0]); });
      bar.appendChild(b);
    });
...
/* ============ POPUP CARDS — Takomo's device, verbatim (Cole, 2026-08-25:
   "Takomo doesn't have a button and they can just click on the image").
   A [data-dlg] card opens the <dialog> it names; native showModal handles
   Esc and focus, the backdrop click is the only part we wire ourselves.
   Lives in core because dialogs may spread beyond the story page; costs
   nothing on pages without a [data-dlg]. */
(function () {
  function brDialogs() {
    var cards = document.querySelectorAll('[data-dlg]');
    if (!cards.length) return;
    [].forEach.call(cards, function (c) {
      c.addEventListener('click', function () {
        var d = document.getElementById(c.getAttribute('data-dlg'));
        if (d && d.showModal) d.showModal();
      });
    });
    [].forEach.call(document.querySelectorAll('dialog.br-dlg'), function (d) {
      d.addEventListener('click', function (e) {
        // a click on the backdrop lands on the dialog element itself;
        // clicks inside land on children
        if (e.target === d) d.close();
      });
      var x = d.querySelector('[data-dlg-close]');
      if (x) x.addEventListener('click', function () { d.close(); });
    });
  }
  document.addEventListener('DOMContentLoaded', brDialogs);
})();
