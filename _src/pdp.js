...
(function(){
  ...
  if (gal){
    ...
    slides.forEach(function(s, i){
      /* A slide can be a labelled placeholder rather than a photograph — the
         tees are the one product with no shot at all, and every apparel page
         is placeholders until the shoot lands. Those have no <img> to build a
         thumbnail from, and a strip of empty boxes is worse than no strip, so
         they are skipped. The thumb carries the slide index it belongs to
         rather than its own position, because skipping one would otherwise
         put every later thumbnail on the wrong slide. */
      ...
      b.innerHTML = '<img src="' + im.getAttribute('src') + '" alt="" loading="lazy">';
      b.addEventListener('click', function(){ show(i); });
      thumbs.appendChild(b);
    });
    var ths = thumbs ? [].slice.call(thumbs.children) : [];
    function show(i){
      at = (i + slides.length) % slides.length;
      slides.forEach(function(s, n){
        if (n === at) s.setAttribute('data-on', ''); else s.removeAttribute('data-on'); });
      ths.forEach(function(t){
        t.setAttribute('aria-current', String(+t.getAttribute('data-i') === at)); });
      var counter = $('#gal-i');
